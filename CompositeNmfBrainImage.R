...
tvalVoxVector[which(tvalVoxVector==1)] <- t[1,1]       		   # for the remaining components, assign p- or t-value (we chose t-values)
tvalVoxVector[which(tvalVoxVector==2)] <- t[2,1]
tvalVoxVector[which(tvalVoxVector==4)] <- t[4,1]
tvalVoxVector[which(tvalVoxVector==7)] <- t[7,1]
tvalVoxVector[which(tvalVoxVector==8)] <- t[8,1]
tvalVoxVector[which(tvalVoxVector==10)] <- t[10,1]
tvalVoxVector[which(tvalVoxVector==18)] <- t[18,1]
tvalVoxVector[which(tvalVoxVector==19)] <- t[19,1]
tvalVoxVector[which(tvalVoxVector==22)] <- t[22,1]
tvalVoxVector[which(tvalVoxVector==23)] <- t[23,1]
tvalVoxVector[which(tvalVoxVector==26)] <- t[26,1]

tvalMap[mask==1]<-as.matrix(tvalVoxVector)			 # essentially this is a replaced image with t-values or 0s instead of comp numbers
antsImageWrite(tvalMap,"/data/joy/BBL/projects/pncPreterm/tablesFigures/NMF_preterm_gaFdrComponents.nii.gz")
...
tvalVoxVector2[which(tvalVoxVector2==1)] <- 2.910    		   # for the remaining components, assign p- or t-value (we chose t-values)
tvalVoxVector2[which(tvalVoxVector2==2)] <- 2.669
tvalVoxVector2[which(tvalVoxVector2==4)] <- 2.377
tvalVoxVector2[which(tvalVoxVector2==8)] <- 2.171
tvalVoxVector2[which(tvalVoxVector2==19)] <- 2.391
tvalVoxVector2[which(tvalVoxVector2==22)] <- 2.321
tvalVoxVector2[which(tvalVoxVector2==26)] <- 2.181
...
antsImageWrite(tvalMap2,"/data/joy/BBL/projects/pncPreterm/tablesFigures/NMF_preterm_mediationFdrComponents.nii.gz")
