####################################################################
#### GAM MODELS WITH AVERAGE MANUAL RATING ADDED AS A COVARIATE ####
####################################################################
...
NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + medu1 + averageManualRating + ga, list(i = as.name(x))), method="REML", data = data.NMF)
})
